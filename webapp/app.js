...
function renderWikiSuggestions(suggestions) {
  const listEl = activeWikiInput === "business" ? el("wikiBusinessSuggestionList") : el("wikiSuggestionList");
  listEl.innerHTML = "";
...
async function updateWikiSuggestions() {
  const query = activeWikiInput === "business" ? el("wikiBusinessInput").value.trim() : el("wikiTermInput").value.trim();
  if (query.length < 2) {
    clearWikiSuggestions();
    return;
  }
