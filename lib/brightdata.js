...
const { collectStringValues, pickFirstByKeys, asTrimmedString, extractNestedString, extractChannelLabel, pickFirstStringByKeys, normalizeDateToYMD } = require("./utils");
...
function parseBrightDataItem(item, fallbackUrl, fallbackVideoId) {
  ...
  // video publish date → "YYYY-MM-DD" ("" when the dataset doesn't carry one).
  // Bright Data's YouTube dataset uses date_posted; the rest are defensive aliases.
  const publishedDate = normalizeDateToYMD(pickFirstByKeys(item, [
    "date_posted", "upload_date", "published_at", "publish_date", "date_published", "release_date"
  ]));
  if (!publishedDate && item && typeof item === "object") {
    // one-line diagnostic so a missing date is debuggable from the server log
    console.warn(`[brightdata] no publish-date field; item keys: ${Object.keys(item).slice(0, 40).join(",")}`);
  }
