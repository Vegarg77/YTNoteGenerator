...
function getConfig() {
  return {
    ...
    OPENAI_MODEL: (process.env.OPENAI_MODEL || "deepseek-v4-flash").trim(),
    // Base URL of the LLM API (any OpenAI-compatible endpoint). Default = DeepSeek.
    // For OpenAI set: https://api.openai.com (model e.g. gpt-4o-mini).
    OPENAI_BASE_URL: (process.env.OPENAI_BASE_URL || "https://api.deepseek.com").replace(/\/+$/, "").trim(),
