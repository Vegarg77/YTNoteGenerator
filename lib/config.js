...
function getConfig() {
  return {
    ...
    // Abort budget for the official MediaWiki lookup (one action=query call). Configurable
    // because TextExtracts is slow on very long articles.
    WIKI_API_TIMEOUT_MS: Number(process.env.WIKI_API_TIMEOUT_MS) || 20000,
    // How often to check whether Google Drive is still running on this machine. The
    // watch dog is here because Drive for desktop can quietly exit on a long-running host
    // (a Windows VM) and stop syncing without ever telling anyone.
    GDRIVE_CHECK_INTERVAL_MS: Number(process.env.GDRIVE_CHECK_INTERVAL_MS) || gdrive.DEFAULT_CHECK_INTERVAL_MS,
  };
}
