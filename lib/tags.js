...
async function scanVaultTags(vaultDir, excludeRaw) {
  ...
  const BATCH = 32;
  const MAX_FILE_BYTES = 2 * 1024 * 1024; // real notes are KB-scale; skip giant exports
  let skippedHuge = 0;
  for (let i = 0; i < files.length; i += BATCH) {
    ...
    for (const text of texts) {
      if (!text) continue;
      if (text.length > MAX_FILE_BYTES) { skippedHuge += 1; continue; }
      ingest(text);
    }
  }
  if (skippedHuge) console.warn(`[tags] skipped ${skippedHuge} file(s) > 2MB`);
