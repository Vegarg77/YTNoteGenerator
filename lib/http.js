...
// retried — a bad title or an API error should surface immediately.
const RETRY_429_DELAYS_MS = [1500, 3000];
...
async function fetchJson(url, { method = "GET", headers = {}, body, signal, label, retryOn429 = false } = {}) {
  const prefix = label ? `${label} ` : "";
  const maxAttempts = retryOn429 ? RETRY_429_DELAYS_MS.length + 1 : 1;

  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const resp = await fetch(url, { method, headers: withDefaultUserAgent(headers), body, signal });

    if (resp.status === 429 && attempt < maxAttempts) {
      await sleep(RETRY_429_DELAYS_MS[attempt - 1], signal);
      continue;
    }
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 429) {
        throw new Error(
          `${prefix}${method} ${url} failed (429): Wikipedia is rate-limiting requests — try again in a moment.`
        );
      }
